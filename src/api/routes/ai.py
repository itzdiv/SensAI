...
from api.utils.safety_filter import safety_filter
...
def get_user_audio_message_for_chat_history(uuid: str) -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"Audio file not found: {uuid}.wav")
        return [
            {
                "type": "text",
                "text": "Student's Response: [Audio file not found]",
            }
        ]
...
async def ai_response_for_question(request: AIChatRequest):
    # Safety check for user response
    task_context = f"Task type: {request.task_type}"
    if request.question:
        question_description = construct_description_from_blocks(request.question.blocks)
        task_context += f", Question: {question_description[:200]}..."  # Limit context length
    
    safety_result = await safety_filter.evaluate_chat_request(
        request.user_response,
        task_context
    )
    
    if not safety_result.is_safe:
        logger.warning(f"Unsafe chat request blocked: {safety_result.reason}")
        raise HTTPException(
            status_code=400,
            detail={
                "error": "Content Safety Violation", 
                "message": "The chat content violates our safety policies.",
                "reason": safety_result.reason,
                "is_safe": False
            }
        )
    
    logger.info(f"Chat request passed safety check: {safety_result.reason}")
...
Do not include the type of task in the name of the task."""
    ...
    # Add the course structure generation prompt
    messages.append({"role": "user", "content": course_structure_generation_prompt})
    ...
    logger.info(f"Sending course_structure_completed event for course {course_id}, job {course_job_uuid}")
    await websocket_manager.send_item_update(
        course_id,
        {
...
async def generate_course_structure(
    ...
    safety_result = await safety_filter.evaluate_course_request(
    ...
    if not safety_result.is_safe:
        logger.warning(f"Unsafe course generation request blocked: {safety_result.reason}")
        raise HTTPException(
            status_code=400,
            detail={
                "error": "Content Safety Violation",
                "message": "The course content request violates our safety policies.",
                "reason": safety_result.reason,
                "is_safe": False
            }
        )
    
    logger.info(f"Course generation request passed safety check: {safety_result.reason}")
    ...
    # Handle reference material if provided
    if request.reference_material_s3_key:
        try:
            # LOCAL ONLY - No S3
            with open(
                os.path.join(
            ...
            logger.error(f"Error processing reference material: {str(e)}")
            # Continue without reference material
            file_id = None

    job_details = {**request.model_dump(), "openai_file_id": file_id}
    job_uuid = await store_course_generation_request(
        course_id,
        job_details,
    )

    async def wrapped_generate_course_structure():
        try:
    ...
    background_tasks.add_task(wrapped_generate_course_structure)

    return {"job_uuid": job_uuid}


@router.post("/safety/check")
async def check_content_safety(content: str = Body(..., embed=True)):
    """
    Test endpoint to check content safety.
    ...
    """
    safety_result = await safety_filter.evaluate_content(content)
